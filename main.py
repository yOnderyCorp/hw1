...
for name, obj in important_stuff.items():
    ...
    if name == "init":
        ...
        print(f"  - Описание: Содержит набор строковых констант для смены цвета. Доступные цвета: {', '.join([a for a in dir(obj) if not a.startswith('_')])}")
    elif name == "Style":
        print("  - Описание: Позволяет изменять яркость (BRIGHT, DIM) или сбрасывать настройки (RESET_ALL).")
    print("-" * 30)

# Пример работы
print(f"\n{colorama.Back.BLUE}{colorama.Fore.WHITE} ПРИМЕР: Синий фон, белый текст {colorama.Style.RESET_ALL}")
